	...
	public class GamePanel extends JPanel implements KeyListener, MouseListener {
		...
		private void drawMenu (Graphics g) {
			...
			// Credits :p
			drawCentered("Code by NTH.hanni", FlappyBird.WIDTH, FlappyBird.HEIGHT, 600, g);
		...
		public void gameOver (Graphics g) {
			...
			// Medal
			if (score >= 1) { medal = "bronze"; }
			if (score >= 2) { medal = "silver"; }
			if (score >= 3) { medal = "gold"; }
			if (score >= 4) { medal = "platinum"; }
			...
			if (gameState == MENU) {
				...
				if (gameBird.isAlive()) {
					...
					if (isTouching(textures.get("rateButton").getRect())) {
						Helper.openURL("https://www.facebook.com/nth.hani"); // Open website
					}
